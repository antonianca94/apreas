...
class Login {
    ...
    public function __construct() {      
        ...
        if ( get_option( 'apreas_eventos_enabled', 1 ) ) {
...
<?php
    ...
    // ============================================================
    // HELPER: atributos comuns de estilização para shortcodes de data
    // ============================================================
    private function atts_data_shortcode($atts, $label_default = '') {
    ...
    private function formatar_whatsapp( $numero ) {
        $n = preg_replace( '/\D/', '', (string) $numero );
        if ( strlen( $n ) === 13 ) {
            return '(' . substr( $n, 2, 2 ) . ') ' . substr( $n, 4, 5 ) . '-' . substr( $n, 9 );
        }
        if ( strlen( $n ) === 12 ) {
            return '(' . substr( $n, 2, 2 ) . ') ' . substr( $n, 4, 4 ) . '-' . substr( $n, 8 );
        }
        return $n;
    }

    function mostrar_imagem_logo_evento() {
        return '<img class="imagem_logo_evento" src="' . esc_url( plugins_url( 'admin/img/placeholder.svg', dirname( __FILE__ ) ) ) . '" alt="Logo do Evento" style="max-width: 100%; height: auto;">';
    }

    function mostrar_imagem_logo_escola() {
        return '<img class="imagem_logo_escola" src="' . esc_url( plugins_url( 'admin/img/placeholder.svg', dirname( __FILE__ ) ) ) . '" alt="Logo da Escola" style="max-width: 100%; height: auto;">';
    }
    ...
    function render_login_form() {
        ...
        $whatsapp_numero = preg_replace( '/\D/', '', get_option( 'apreas_whatsapp_numero', '5511939490911' ) );
        $whatsapp_exibicao = $this->formatar_whatsapp( $whatsapp_numero );
        $cor_primaria    = get_option( 'apreas_primary_color', '#d32f2f' );
            ...
            <div id="loginContent">
                ...
                <form class="row g-3" id="form_login" name="form_login" enctype="multipart/form-data">
                    ...
                    <div class="col-12 duvidas-box">
                        ...
                        <a href="https://wa.me/<?php echo esc_attr( $whatsapp_numero ); ?>" target="_blank" class="duvidas-whatsapp">
                            ...
                            <?php echo esc_html( $whatsapp_exibicao ); ?> &nbsp;<em>WhatsApp</em>
                        </a>
                    </div>                     
        ...
        <style>
            ...
            .btn_form_login {
                background: <?php echo esc_attr( $cor_primaria ); ?> !important;
                color: #fff !important;
                font-weight: 500 !important;
                border: 2px solid <?php echo esc_attr( $cor_primaria ); ?> !important;
                text-transform: uppercase !important;
                margin-top: 8px;
